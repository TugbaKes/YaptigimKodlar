...
$satir_kattek = $sorgu_kattek->fetch() //Tek bir sorguyu tek bir satıra çevirir
?>
            ...
            <div class="col-md-4">
                ...
                <form method="post">
                    <div class="form-group">
                        <input type="text" name="kat_adi" value="<?php echo $satir_kattek['kat_adi']; ?>" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <small><label>Kategori Türü - <?php echo $satir_kattek['kat_turu']; ?></label></small>
                    ...
                    <div class="form-group">
                        <textarea name="aciklama" rows="5" class="form-control"><?php echo $satir_kattek['aciklama']; ?></textarea>
                    </div>
                    <div class="form-group">
                ...
                <?php
                //Diğer bir formatta güncelleyerek kaydetme işlemi (Değişkenler oluşturulmadı)
                if ($_POST) {
                    $sorgu_guncelkat = $db->prepare('update kategoriler set kat_adi=?, kat_turu=?, aciklama=? where id=?');
                    $sorgu_guncelkat->execute(array($_POST['kat_adi'], $_POST['kat_turu'], $_POST['aciklama'],$id));

                    if($sorgu_guncelkat -> rowCount()){
                        echo '<div class="alert alert-success">Kayıt Güncellenmiştir</div><meta http-equiv="refresh" content="1; url=kategori.php">';
                    } else {
